<script lang="ts">
  ...
  const close = () => tooltip.classList.add('hidden')
  ...
  $: anchor && tooltip && doCompute()
</script>

<svelte:body on:scroll={() => anchor && tooltip && doCompute()} />
...
<div
  bind:this={tooltip}
  class="rounded absolute text-error-600-300-token hidden bg-surface-800 shadow-lg"
...
>
  <div class="variant-soft-error p-8 space-y-8 rounded border-2 border-error-500">
    ...
    <div class="buttons">
      <button type="button" class="btn rounded variant-ghost-error" on:click={close}>Cancel</button>
